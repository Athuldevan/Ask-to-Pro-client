...
import { useEditProfileMutation } from "@/lib/slices/profileApi";
import { useState, type ChangeEvent } from "react";

export default function ProfilePage() {
  const [isEditMode, setIsEditMode] = useState(true);
  const { data: user } = useViewProfileQuery(undefined);
  ...
  async function handleSave() {
    await editProfile(editForm).unwrap();
    setIsEditMode(false);
  }
          ...
          <Card className="col-span-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-md rounded-2xl">
            <CardContent className="p-8 space-y-8">
              ...
              <Button
                disabled={isLoading}
                onClick={handleSave}
                className="bg-[#7e22ce] hover:bg-[#6c1fa5] text-white px-6 py-3 rounded-xl"
              >
                Save Changes
              </Button>
